...
- **Below `liquidationCursor * (1 - LLTV)` (= `1 - 1 / maxLif`) for collaterals liquidated through `MidnightVaultExecutor`.** The executor funds the repayment strictly from redeeming the seized shares, so it only tolerates oracle-above-redemption skew up to the liquidation bonus: beyond it, every executor liquidation reverts with `RepayExceedsRedeemed` while `price()` still passes. This ceiling is stricter than `(1 - LLTV * maxLif)` whenever `liquidationCursor < 1 / (1 + sqrt(LLTV))` (about `0.51` at 90% LLTV). With the validation feed as the proxy for the redemption rate, size the deviation below `liquidationCursor * (1 - LLTV)` for these collaterals, with the same headroom for honest drift — or raise the cursor if that leaves no room. Even an in-band skew delays post-maturity settlement-mode liquidations until the incentive ramp exceeds `1 / (1 - skew)`, up to the full 60-minute ramp.
