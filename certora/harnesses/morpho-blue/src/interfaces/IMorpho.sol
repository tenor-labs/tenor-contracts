...
/// interest accrual.
/// @dev FV note: this is the Certora-only fork of morpho-blue. The struct upstream morpho-blue (and
/// `lib/morpho-blue`) calls `Market` is renamed here to `MarketBlue` to avoid a duplicate-type merge with
/// midnight's `struct Market` when a Certora scene loads both protocols (Tenor's Blue-callback confs). The
/// stock `lib/morpho-blue` keeps the upstream `Market` name; Certora confs remap `@morphoBlue` to this tree.
struct MarketBlue {
...
interface IMorpho is IMorphoBase {
    ...
    /// interest accrual.
    function market(Id id) external view returns (MarketBlue memory m);
