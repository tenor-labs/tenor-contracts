...
| ID | Property | Description |
|----|----------|-------------|
| GATE-5 | **Fee recipient auto-allow** | `canReceiveShares(account)` returns `true` if `account` is the vault's `managementFeeRecipient()` or `performanceFeeRecipient()`, even if not explicitly allowlisted. This prevents fees from silently accruing as zero. Note: the effective receive-share set is only fully frozen if BOTH the gate is renounced AND the vault's fee-recipient setters (`setManagementFeeRecipient`, `setPerformanceFeeRecipient`) are abdicated upstream — a vault curator with active setters can post-renounce nominate any address as fee recipient, making it share-eligible. |
