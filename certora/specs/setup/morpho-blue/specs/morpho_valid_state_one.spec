// Valid-state invariants for Morpho Blue (one-market regime).
//
// Each per-market invariant is proven for a single symbolic market under the
// one-market narrowing; because markets do not interfere, that proof
// generalises to every market. The many-market regime
// (morpho_valid_state_many.spec) additionally re-verifies the parametric
// forall id forms.

import "./setup/morpho_one.spec";

// ========== SETUP ==========

function setupValidStateOneBlue(env e) {
    setupOneBlue(e);
...
function SETUP_ONE_BLUE(env e, env eFunc) {
...
// FORMULA: ghostMbOneFee128 <= MAX_FEE_CVL()
//
...
invariant feeBounded(env e)
    ghostMbOneFee128 <= MAX_FEE_CVL()
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-02: Fee requires market existence
// FORMULA: ghostMbOneFee128 != 0 => ghostMbOneLastUpdate128 != 0
//
// setFee requires market[id].lastUpdate != 0. createMarket leaves fee at 0.
invariant feeRequiresMarket(env e)
    ghostMbOneFee128 != 0 => ghostMbOneLastUpdate128 != 0
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-03: Last update bounded by block timestamp
// FORMULA: ghostMbOneLastUpdate128 != 0 => ghostMbOneLastUpdate128 <= to_mathint(e.block.timestamp)
//
...
invariant lastUpdateBoundedByTimestamp(env e)
    ghostMbOneLastUpdate128 != 0
    => ghostMbOneLastUpdate128 <= to_mathint(e.block.timestamp)
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-04: Last update minimum bound
// FORMULA: ghostMbOneLastUpdate128 != 0 => ghostMbOneLastUpdate128 >= MIN_BLOCK_TIMESTAMP()
//
...
invariant lastUpdateMinBound(env e)
    ghostMbOneLastUpdate128 != 0
    => ghostMbOneLastUpdate128 >= MIN_BLOCK_TIMESTAMP()
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-05: Liquidity invariant -- total borrow <= total supply
// FORMULA: ghostMbOneLastUpdate128 != 0 => ghostMbOneTotalBorrowAssets128 <= ghostMbOneTotalSupplyAssets128
//
...
invariant liquidityInvariant(env e)
    ghostMbOneLastUpdate128 != 0
    => ghostMbOneTotalBorrowAssets128 <= ghostMbOneTotalSupplyAssets128
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-06: Supply shares solvency -- total >= sum of individual positions
// FORMULA: ghostMbOneTotalSupplyShares128 >= SUPPLY_SHARES_SUM_ONE()
//
...
invariant supplySharesSolvency(env e)
    ghostMbOneTotalSupplyShares128 >= SUPPLY_SHARES_SUM_ONE()
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-07: Borrow shares solvency -- total >= sum of individual positions
// FORMULA: ghostMbOneTotalBorrowShares128 >= BORROW_SHARES_SUM_ONE()
//
...
invariant borrowSharesSolvency(env e)
    ghostMbOneTotalBorrowShares128 >= BORROW_SHARES_SUM_ONE()
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-08: Non-existent market has zero accounting
// FORMULA: ghostMbOneLastUpdate128 == 0
...
invariant nonExistentMarketIsZero(env e)
    ghostMbOneLastUpdate128 == 0
    => (ghostMbOneTotalSupplyAssets128 == 0
        && ghostMbOneTotalSupplyShares128 == 0
        && ghostMbOneTotalBorrowAssets128 == 0
        && ghostMbOneTotalBorrowShares128 == 0
        && ghostMbOneFee128 == 0)
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }
...
invariant nonExistentMarketParamsAreZero(env e)
    ghostMbOneLastUpdate128 == 0
    => (ghostMbOneLoanToken == 0
        && ghostMbOneCollateralToken == 0
        && ghostMbOneOracle == 0
        && ghostMbOneIrm == 0
        && ghostMbOneLltv256 == 0)
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }
...
// FORMULA: ghostMbOneLastUpdate128 != 0 => ghostMbIsIrmEnabled[ghostMbOneIrm]
//
...
invariant marketIrmIsEnabled(env e)
    ghostMbOneLastUpdate128 != 0
    => ghostMbIsIrmEnabled[ghostMbOneIrm]
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-11: Market LLTV is enabled
// FORMULA: forall lltv. (lastUpdate != 0 && lltv == ghostMbOneLltv256) => isLltvEnabled[lltv]
//
...
invariant marketLltvIsEnabled(env e)
    forall uint256 lltv.
        (ghostMbOneLastUpdate128 != 0 && ghostMbOneLltv256 == to_mathint(lltv))
...
// FORMULA: ghostMbOneLastUpdate128 != 0 => ghostMbOneLltv256 < MORPHO_WAD_CVL()
//
...
invariant marketLltvBelowWad(env e)
    ghostMbOneLastUpdate128 != 0
    => ghostMbOneLltv256 < MORPHO_WAD_CVL()
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-14: Supply shares require market existence
// FORMULA: forall user. ghostMbOneSupplyShares256[user] > 0 => ghostMbOneLastUpdate128 != 0
//
...
invariant supplySharesRequiresMarket(env e)
    forall address user.
        ghostMbOneSupplyShares256[user] > 0
        => ghostMbOneLastUpdate128 != 0
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-15: Borrow shares require market existence
// FORMULA: forall user. ghostMbOneBorrowShares128[user] > 0 => ghostMbOneLastUpdate128 != 0
//
// borrow() requires market[id].lastUpdate != 0. liquidate only operates
// on existing markets.
invariant borrowSharesRequiresMarket(env e)
    forall address user.
        ghostMbOneBorrowShares128[user] > 0
        => ghostMbOneLastUpdate128 != 0
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-16: Collateral requires market existence
// FORMULA: forall user. ghostMbOneCollateral128[user] > 0 => ghostMbOneLastUpdate128 != 0
//
...
invariant collateralRequiresMarket(env e)
    forall address user.
        ghostMbOneCollateral128[user] > 0
        => ghostMbOneLastUpdate128 != 0
filtered { f -> !EXCLUDED_FUNCTION_MB(f) } { preserved with (env eFunc) { SETUP_ONE_BLUE(e, eFunc); } }

// VS-17: Total supply assets require market existence
// FORMULA: ghostMbOneTotalSupplyAssets128 > 0 => ghostMbOneLastUpdate128 != 0
//
...
// FORMULA: ghostMbOneTotalSupplyShares128 > 0 => ghostMbOneLastUpdate128 != 0
//
...
// FORMULA: ghostMbOneTotalBorrowAssets128 > 0 => ghostMbOneLastUpdate128 != 0
//
...
// FORMULA: ghostMbOneTotalBorrowShares128 > 0 => ghostMbOneLastUpdate128 != 0
//
...
// FORMULA: forall user. ghostMbOneLastUpdate128 == 0 => (supplyShares == 0 && borrowShares == 0 && collateral == 0)
//
...
// FORMULA: forall user. ghostMbOneBorrowShares128[user] != 0 => ghostMbOneCollateral128[user] != 0
//
