// perf kill overlay BorrowMidnightRenewalCallback/many: branches B10, B3 disabled — per the footprint matrix
// they do not intersect the read-set of the listed rule — plus a single-collateral kill-scene pin.
// RULES: borrowerFeeBoundedByInterestShare (mutation-kill channel: BMR#36)
// BASE: light
// STUBS: B10,B3
// PINS: ghostNumCollaterals==1 (kill scene; assert-CEX channel, so a found CEX is a genuine trace)
// SATISFY-TWINS (re-check non-vacuity under the same overlay): borrowerFeeBoundedByInterestShare
...
override function touchMarketCVL(env e, MidnightHarness.Market market) returns bytes32 {
    // K1 PIN: the BMR#36 kill needs only the fee delta at the feeRecipient, which is
    // collateral-count-agnostic; otherwise the transferCollaterals per-slot pro-rata mulDiv
    // fan-out explodes JVM split generation before any CEX is attempted. Assert-CEX channel:
    // the pin can only hide a CEX (miss), never fabricate one, so it is safe-by-construction.
    require(ghostNumCollaterals == 1,
        "SCOPE: single-collateral kill scene (BMR#36) — fee-bound CEX is collateral-count-agnostic");
...
use rule borrowerFeeBoundedByInterestShare;
use rule borrowerFeeBoundedByInterestShare__satisfy;
