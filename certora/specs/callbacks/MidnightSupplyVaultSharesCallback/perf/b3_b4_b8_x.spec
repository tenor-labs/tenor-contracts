// perf overlay MidnightSupplyVaultSharesCallback/one: branches B3 (partial), B4, B8 disabled — per the footprint matrix
...
// RULES: onlyVaultSlotReceivesSupply suppliedSharesMatchMintedShares
// BASE: light
// STUBS: B3(partial: withdrawCollateral only, see B3-fix below),B4,B8
// B7-ELIGIBLE (hashing flags, deferred to keccak inventory): onlyVaultSlotReceivesSupply
// SATISFY-TWINS (re-check non-vacuity under the same overlay): onlyVaultSlotReceivesSupply suppliedSharesMatchMintedShares
...
methods {
    // B3-fix: supplyCollateral is NOT stubbed — it writes the seller's collateral that
    // suppliedSharesMatchMintedShares asserts against; NONDET here skipped that write and
    // produced a spurious CEX (minted shares != 0 but collateral delta == 0).
...
use rule onlyVaultSlotReceivesSupply;
use rule suppliedSharesMatchMintedShares;
use rule onlyVaultSlotReceivesSupply__satisfy;
use rule suppliedSharesMatchMintedShares__satisfy;
