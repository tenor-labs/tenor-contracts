// perf kill overlay LendMidnightRenewalCallback/many: branches B2, B3, B4, B8 disabled — per the footprint matrix
...
// — plus a single-collateral kill-scene pin (K1). This is the base perf spec b2_b3_b4_b8.spec verbatim + the K1 pin.
// RULES: renewalCallbackNeverPullsExternalLoanToken renewalNeverTouchesUnrelatedLenderCredit (mutation-kill channel: LMR#25, satisfy twin)
// BASE: light
// STUBS: B2,B3,B4,B8
// PINS: ghostNumCollaterals==1 (kill scene; ghostNumTicks==1 already pinned in many_setup)
// B7-ELIGIBLE (hashing flags, deferred to keccak inventory): renewalCallbackNeverPullsExternalLoanToken renewalNeverTouchesUnrelatedLenderCredit
// SATISFY-TWINS (re-check non-vacuity under the same overlay): renewalCallbackNeverPullsExternalLoanToken renewalNeverTouchesUnrelatedLenderCredit
...
override function touchMarketCVL(env e, MidnightHarness.Market market) returns bytes32 {
    // K1 PIN: the LMR#25 kill argument lives entirely in the buyerAssets allowance path
    // (forceApprove(buyerAssets - 1) => Midnight's pull of the full buyerAssets exceeds the allowance and
    // reverts on every path => take() always reverts => the satisfy witness is unsatisfiable), which is
    // collateral-count-agnostic; the LMR onBuy body itself is collateral-free (lender credit only), so a
    // single collateral fully expresses the "unrelated lender credit untouched" witness. The pin only trims
    // the parametric take grid that inflates JVM split generation.
    ...
    require(ghostNumCollaterals == 1,
        "SCOPE: single-collateral kill scene (LMR#25) — unrelated-lender-credit witness is collateral-count-agnostic");
...
use rule renewalCallbackNeverPullsExternalLoanToken;
use rule renewalNeverTouchesUnrelatedLenderCredit;
use rule renewalCallbackNeverPullsExternalLoanToken__satisfy;
use rule renewalNeverTouchesUnrelatedLenderCredit__satisfy;
