// perf overlay LendMidnightRenewalCallback/many: branches B12, B2, B3, B4, B8 disabled — per the footprint matrix
...
// RULES: callbackRevertsForNonMidnightCaller callbackRevertsForSameSourceMarket callbackRevertsOnZeroAssetsOrUnits
// BASE: light
// STUBS: B12,B2,B3,B4,B8
// B7-ELIGIBLE (hashing flags, deferred to keccak inventory): callbackRevertsForNonMidnightCaller
// SATISFY-TWINS (re-check non-vacuity under the same overlay):
...
methods {
    function MidnightHarness.withdraw(MidnightHarness.Market market, uint256 units,
        address onBehalf, address receiver) external => NONDET;
...
// B12: credit-redemption withdraw -> NONDET (S1 exact-receiver; no storage writes).
...
use rule callbackRevertsForNonMidnightCaller;
use rule callbackRevertsForSameSourceMarket;
use rule callbackRevertsOnZeroAssetsOrUnits;
