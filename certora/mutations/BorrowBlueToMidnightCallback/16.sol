/* ── MUTATION BorrowBlueToMidnightCallback #16 ──────────────────────────────
 * @desc:   Replacing the Blue collateral withdrawal with a supply makes the seller's old Blue collateral grow instead of shrink, so the rule requiring migration to only withdraw old Blue collateral produces a counterexample.
 * @rules:  migrationOnlyWithdrawsOldBlueCollateral
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/perf/migrationOnlyWithdrawsOldBlueCollateral.conf
 * @status: killed
 * @target: src/callbacks/BorrowBlueToMidnightCallback.sol
 * Was:     MORPHO_BLUE.withdrawCollateral(sourceMarketParams, collateralMigrated, seller, address(this));
 * Now:     MORPHO_BLUE.supplyCollateral(sourceMarketParams, collateralMigrated, seller, "");
 * ────────────────────────────────────────────────────────────────────*/
