/* ── MUTATION BorrowBlueToMidnightCallback #9 ──────────────────────────────
 * @desc:   Supplying collateral to the new Midnight market is replaced by withdrawing it, so the Midnight collateral shrinks instead of growing and the rule requiring migration to only add new-market collateral produces a counterexample.
 * @rules:  migrationOnlyAddsNewMidnightCollateral
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/perf/migrationOnlyAddsNewMidnightCollateral.conf
 * @status: killed
 ...
 * Now:     MORPHO_MIDNIGHT.withdrawCollateral(market, collateralIndex, collateralMigrated, seller, address(this));
 * ────────────────────────────────────────────────────────────────────*/
