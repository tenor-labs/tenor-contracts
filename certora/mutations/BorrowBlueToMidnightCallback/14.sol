/* ── MUTATION BorrowBlueToMidnightCallback #14 ──────────────────────────────
 * @desc:   Flipping the source-market loan-token check from not-equal to equal accepts a source market whose loan token differs from the offer's, so the rule requiring a mismatched loan token to revert produces a counterexample.
 * @rules:  sourceLoanTokenMismatchReverts
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/sourceLoanTokenMismatchReverts.conf
 * @status: killed
 * @target: src/callbacks/BorrowBlueToMidnightCallback.sol
 * Was:     if (sourceMarketParams.loanToken != loanToken) revert CallbackLib.TokenMismatch();
 * Now:     if (sourceMarketParams.loanToken == loanToken) revert CallbackLib.TokenMismatch();
 * ────────────────────────────────────────────────────────────────────*/
