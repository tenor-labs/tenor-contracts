/* ── MUTATION BorrowBlueToMidnightCallback #21 ──────────────────────────────
 * @desc:   Migrating one wei less than the full Blue collateral on the final fill always leaves a wei behind, so the seller's collateral never reaches zero and the witness showing the old position can be fully closed can no longer be produced.
 * @rules:  migrationCanFullyCloseOldPosition
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/perf_kill/migrationCanFullyCloseOldPosition.conf
 * @status: killed
 * @target: src/callbacks/BorrowBlueToMidnightCallback.sol
 * Was:     uint256 collateralMigrated = isFinalFill ? blueCollateral : blueCollateral.mulDivDown(repayBudget, blueDebt);
 * Now:     uint256 collateralMigrated = isFinalFill ? blueCollateral - 1 : blueCollateral.mulDivDown(repayBudget, blueDebt);
 * ────────────────────────────────────────────────────────────────────*/
