/* ── MUTATION BorrowBlueToMidnightCallback #12 ──────────────────────────────
 * @desc:   Changing the excess-repayment check from greater-than to greater-or-equal makes the final fill (where repayBudget equals the debt) revert, so the witness showing all Blue collateral fully withdrawn can no longer be produced.
 * @rules:  fullCollateralMigrationClearsAllOldDebt__satisfy
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/perf_kill_satisfy/fullCollateralMigrationClearsAllOldDebt.conf
 * @status: killed
 * @target: src/callbacks/BorrowBlueToMidnightCallback.sol
 * Was:     if (repayBudget > blueDebt) revert CallbackLib.ExcessRepayment();
 * Now:     if (repayBudget >= blueDebt) revert CallbackLib.ExcessRepayment();
 * ────────────────────────────────────────────────────────────────────*/
