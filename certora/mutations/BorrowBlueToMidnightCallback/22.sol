/* ── MUTATION BorrowBlueToMidnightCallback #22 ──────────────────────────────
 * @desc:   onSell receiver guard flipped (!= -> ==): receiver!=callback no longer reverts. Caught by receiverNotCallbackReverts (CLB-BBM-22, receiverNotCallback => reverted via callbackCallWithRevert) — the flip leaves a non-reverting receiver!=callback trace, assert violated.
 * @rules:  receiverNotCallbackReverts
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/receiverNotCallbackReverts.conf
 * @status: killed
