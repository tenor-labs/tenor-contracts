/* ── MUTATION BorrowBlueToMidnightCallback #15 ──────────────────────────────
 * @desc:   Replacing the Blue repay with a borrow makes the seller's old Blue debt increase instead of decrease, so the rule requiring migration to only reduce the old Blue debt produces a counterexample.
 * @rules:  migrationOnlyReducesOldBlueDebt
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/perf/migrationOnlyReducesOldBlueDebt.conf
 * @status: killed
 * @target: src/callbacks/BorrowBlueToMidnightCallback.sol
 * Was:     MORPHO_BLUE.repay(sourceMarketParams, repayBudget, 0, seller, "");
 * Now:     MORPHO_BLUE.borrow(sourceMarketParams, repayBudget, 0, seller, seller);
 * ────────────────────────────────────────────────────────────────────*/
