/* ── MUTATION BorrowBlueToMidnightCallback #18 ──────────────────────────────
 * @desc:   Doubling the amount transferred to the fee recipient makes the borrower pay twice the intended fee, so the rule bounding the borrower fee by its interest share produces a counterexample.
 * @rules:  borrowerFeeBoundedByInterestShare
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/perf/borrowerFeeBoundedByInterestShare.conf
 * @status: killed
 * @target: src/callbacks/BorrowBlueToMidnightCallback.sol
 * Was:     SafeTransferLib.safeTransfer(loanToken, callbackData.feeRecipient, fee);
 * Now:     SafeTransferLib.safeTransfer(loanToken, callbackData.feeRecipient, fee * 2);
 * ────────────────────────────────────────────────────────────────────*/
