/* ── MUTATION BorrowBlueToMidnightCallback #20 ──────────────────────────────
 * @desc:   Flipping the caller check from not-equal to equal makes onSell revert whenever Midnight (its only legitimate caller) invokes it, so take() always reverts and the witness that migration reduces old debt on at most one market can no longer be produced.
 * @rules:  migrationReducesOldDebtOnAtMostOneMarket__satisfy
 * @conf:   certora/confs/callbacks/BorrowBlueToMidnightCallback/debug_satisfy/migrationReducesOldDebtOnAtMostOneMarket.conf
 * @status: killed
