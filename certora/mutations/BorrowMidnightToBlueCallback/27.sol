/* ── MUTATION BorrowMidnightToBlueCallback #27 ──────────────────────────────
 * @desc:   Replacing the Blue borrow call with repay makes the borrower's new Blue debt shares fall instead of rise, so the rule that the migration can only open new Blue debt flips to a counterexample.
 * @rules:  migrationOnlyOpensNewBlueDebt
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/perf/migrationOnlyOpensNewBlueDebt.conf
 * @status: killed
 ...
 * Now:     MORPHO_BLUE.repay(callbackData.targetMarketParams, borrowAmount, 0, buyer, "");
 * ────────────────────────────────────────────────────────────────────*/
