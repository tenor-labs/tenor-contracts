/* ── MUTATION BorrowMidnightToBlueCallback #30 ──────────────────────────────
 * @desc:   Borrowing on behalf of the callback contract instead of the buyer opens new Blue debt for a party whose old Midnight debt did not fall, so the rule coupling old and new debt movements flips to a counterexample.
 * @rules:  oldMidnightDebtAndNewBlueDebtMoveTogether
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/perf/oldMidnightDebtAndNewBlueDebtMoveTogether.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightToBlueCallback.sol
 * Was:     MORPHO_BLUE.borrow(callbackData.targetMarketParams, borrowAmount, 0, buyer, address(this));
 * Now:     MORPHO_BLUE.borrow(callbackData.targetMarketParams, borrowAmount, 0, address(this), address(this));
 * ────────────────────────────────────────────────────────────────────*/
