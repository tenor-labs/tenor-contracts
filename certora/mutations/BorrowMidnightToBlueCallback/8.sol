/* ── MUTATION BorrowMidnightToBlueCallback #8 ──────────────────────────────
 * @desc:   Commenting out the borrow call prevents Blue debt from being opened, making the satisfy clause impossible to prove (cannot demonstrate that blueSharesAfter > sharesBefore).
 * @rules:  migrationCanOpenNewBlueDebt
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/migrationCanOpenNewBlueDebt.conf
 * @status: killed
 ...
 * Now:     // MORPHO_BLUE.borrow(callbackData.targetMarketParams, borrowAmount, 0, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
