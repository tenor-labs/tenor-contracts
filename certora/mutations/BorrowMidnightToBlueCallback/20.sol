/* ── MUTATION BorrowMidnightToBlueCallback #20 ──────────────────────────────
 * @desc:   Replacing the Midnight withdrawCollateral call with supplyCollateral makes the borrower's old Midnight collateral grow instead of shrink, so the rule that the migration can only reduce old collateral flips to a counterexample.
 * @rules:  migrationOnlyWithdrawsOldMidnightCollateral
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/perf/migrationOnlyWithdrawsOldMidnightCollateral.conf
 * @status: killed
 ...
 * Now:     MORPHO_MIDNIGHT.supplyCollateral(market, collateralIndex, collateralMigrated, buyer);
 * ────────────────────────────────────────────────────────────────────*/
