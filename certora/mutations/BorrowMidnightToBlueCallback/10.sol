/* ── MUTATION BorrowMidnightToBlueCallback #10 ──────────────────────────────
 * @desc:   Setting the amount withdrawn from Midnight to zero leaves the borrower's old collateral in place, so no fill can ever fully close the old position and the rule's witness becomes unsatisfiable.
 * @rules:  migrationCanFullyCloseOldPosition
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/migrationCanFullyCloseOldPosition.conf
 * @status: killed
 ...
 * Now:     MORPHO_MIDNIGHT.withdrawCollateral(market, collateralIndex, 0, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
