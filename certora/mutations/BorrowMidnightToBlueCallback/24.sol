/* ── MUTATION BorrowMidnightToBlueCallback #24 ──────────────────────────────
 * @desc:   Withdrawing one unit less from Midnight than is supplied to Blue makes the amount deposited into Blue exceed the amount withdrawn from Midnight, so the deposit-at-most-withdrawn rule flips to a counterexample.
 * @rules:  migrationCannotDepositMoreCollateralThanWithdrawn
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/perf/migrationCannotDepositMoreCollateralThanWithdrawn.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightToBlueCallback.sol
 * Was:     MORPHO_MIDNIGHT.withdrawCollateral(market, collateralIndex, collateralMigrated, buyer, address(this));
 * Now:     MORPHO_MIDNIGHT.withdrawCollateral(market, collateralIndex, collateralMigrated - 1, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
