/* ── MUTATION BorrowMidnightToBlueCallback #29 ──────────────────────────────
 * @desc:   Inverts the caller guard (!= to ==), so onBuy reverts OnlyMidnight on every take() (which always comes from Midnight) and the reachability witness goes UNSAT.
 * @rules:  migrationReducesOldDebtOnAtMostOneMarket__satisfy
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/debug_satisfy/migrationReducesOldDebtOnAtMostOneMarket.conf
 * @status: killed
