/* ── MUTATION BorrowMidnightToBlueCallback #26 ──────────────────────────────
 * @desc:   Replacing the Blue supplyCollateral call with withdrawCollateral makes the borrower's new Blue collateral shrink instead of grow, so the rule that the migration can only add new Blue collateral flips to a counterexample.
 * @rules:  migrationOnlyAddsNewBlueCollateral
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/perf/migrationOnlyAddsNewBlueCollateral.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightToBlueCallback.sol
 * Was:     MORPHO_BLUE.supplyCollateral(callbackData.targetMarketParams, collateralMigrated, buyer, "");
 * Now:     MORPHO_BLUE.withdrawCollateral(callbackData.targetMarketParams, collateralMigrated, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
