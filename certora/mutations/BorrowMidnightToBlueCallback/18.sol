/* ── MUTATION BorrowMidnightToBlueCallback #18 ──────────────────────────────
 * @desc:   fee-cap guard inverted (> -> <): every legal below-cap feeRate now reverts while an above-cap rate is accepted — percentageFeeRateAboveCapReverts (aboveCap => reverted) is violated.
 * @rules:  percentageFeeRateAboveCapReverts
 * @conf:   certora/confs/callbacks/BorrowMidnightToBlueCallback/percentageFeeRateAboveCapReverts.conf
 * @status: killed
 * @target: src/libraries/CallbackLib.sol
 * Was:     if (feeRate > MAX_PERCENTAGE_FEE_RATE) revert InvalidFeeConfig();
 * Now:     if (feeRate < MAX_PERCENTAGE_FEE_RATE) revert InvalidFeeConfig();
 * ────────────────────────────────────────────────────────────────────*/
