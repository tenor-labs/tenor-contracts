/* ── MUTATION MigrationRatifier #12 ──────────────────────────────
 * @desc:   isRatified key swap [src][tgt]->[tgt][src]: reads a non-addressed tuple
 * @rules:  isRatifiedReadsOnlyAddressedParams
 * @conf:   certora/confs/ratifier/highlevel.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     UserMigrationParams memory params = userParams[offer.maker][offer.callback][src][tgt];
 * Now:     UserMigrationParams memory params = userParams[offer.maker][offer.callback][tgt][src];
 * ────────────────────────────────────────────────────────────────────*/
