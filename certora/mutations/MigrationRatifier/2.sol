/* ── MUTATION MigrationRatifier #2 ──────────────────────────────
 * @desc:   Comment out the assignment so setParams does not actually write the tuple, breaking the storage fidelity invariant
 * @rules:  setParamsWritesTupleAndLeavesOthers
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     userParams[onBehalf][callback][sourceTenorMarketId][targetTenorMarketId] = params;
 * Now:     // userParams[onBehalf][callback][sourceTenorMarketId][targetTenorMarketId] = params;
 * ────────────────────────────────────────────────────────────────────*/
