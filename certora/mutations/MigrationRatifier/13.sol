/* ── MUTATION MigrationRatifier #13 ──────────────────────────────
 * @desc:   Replace the success-token return with bytes32(0) so an accepting isRatified no longer produces the CALLBACK_SUCCESS value take() requires
 * @rules:  isRatifiedReturnsCallbackSuccess
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     return CALLBACK_SUCCESS;
 * Now:     return bytes32(0);
 * ────────────────────────────────────────────────────────────────────*/
