/* ── MUTATION MigrationRatifier #9 ──────────────────────────────
 * @desc:   invalid-length guard != -> == : accepts non-64-byte ratifierData
 * @rules:  invalidRatifierDataLengthReverts
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     if (ratifierData.length != 64) revert InvalidRatifierData();
 * Now:     if (ratifierData.length == 64) revert InvalidRatifierData();
 * ────────────────────────────────────────────────────────────────────*/
