/* ── MUTATION MigrationRatifier #10 ──────────────────────────────
 * @desc:   pinned-receiver guard != -> == : accepts unpinned receiver
 * @rules:  makerReceiverMustBePinned
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     if (offer.receiverIfMakerIsSeller != (offer.buy ? address(0) : offer.callback)) revert InvalidReceiver();
 * Now:     if (offer.receiverIfMakerIsSeller == (offer.buy ? address(0) : offer.callback)) revert InvalidReceiver();
 * ────────────────────────────────────────────────────────────────────*/
