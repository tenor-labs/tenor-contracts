/* ── MUTATION MigrationRatifier #8 ──────────────────────────────
 * @desc:   auth guard short-circuited to false: caller can change params on behalf of an unauthorizing owner
 * @rules:  userParamsChangeRequiresAuthorization
 * @conf:   certora/confs/ratifier/access_control.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     if (msg.sender != onBehalf && !MORPHO_MIDNIGHT.isAuthorized(onBehalf, msg.sender)) {
 * Now:     if (false && msg.sender != onBehalf && !MORPHO_MIDNIGHT.isAuthorized(onBehalf, msg.sender)) {
 * ────────────────────────────────────────────────────────────────────*/
