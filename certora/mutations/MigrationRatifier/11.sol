/* ── MUTATION MigrationRatifier #11 ──────────────────────────────
 * @desc:   group-namespace guard != -> == : accepts out-of-namespace group
 * @rules:  migrationGroupNamespaceEnforced
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     if ((offer.group & MIGRATION_GROUP_HEADER_MASK) != MIGRATION_GROUP_HEADER) revert InvalidGroup();
 * Now:     if ((offer.group & MIGRATION_GROUP_HEADER_MASK) == MIGRATION_GROUP_HEADER) revert InvalidGroup();
 * ────────────────────────────────────────────────────────────────────*/
