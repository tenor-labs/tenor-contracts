/* ── MUTATION MigrationRatifier #3 ──────────────────────────────
 * @desc:   Comment out the delete statement so clearParams does not actually zero the tuple, breaking the clear invariant
 * @rules:  clearParamsZeroesTupleAndLeavesOthers
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/ratifiers/MigrationRatifier.sol
 * Was:     delete userParams[onBehalf][callback][sourceTenorMarketId][targetTenorMarketId];
 * Now:     // delete userParams[onBehalf][callback][sourceTenorMarketId][targetTenorMarketId];
 * ────────────────────────────────────────────────────────────────────*/
