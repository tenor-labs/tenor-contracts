/* ── MUTATION MidnightSupplyVaultSharesCallback #22 ──────────────────────────────
 * @desc:   Supplying the vault shares on behalf of the loan token address instead of the seller credits an unrelated third account's Midnight position, so a bystander's collateral rises and the rule that a supply take never touches a bystander's position produces a counterexample.
 * @rules:  bystanderUntouched
 * @conf:   certora/confs/callbacks/MidnightSupplyVaultSharesCallback/bystanderUntouched.conf
 * @status: killed
 ...
 * Now:     MORPHO_MIDNIGHT.supplyCollateral(market, callbackData.collateralIndex, shares, loanToken);
 * ────────────────────────────────────────────────────────────────────*/
