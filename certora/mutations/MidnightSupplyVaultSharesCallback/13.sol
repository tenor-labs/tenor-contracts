/* ── MUTATION MidnightSupplyVaultSharesCallback #13 ──────────────────────────────
 * @desc:   inserted unconditional seller pull : loanToken is pulled from the seller even when additionalDepositPercent == 0
 * @rules:  noExtraPullWhenPercentZero
 * @conf:   certora/confs/callbacks/MidnightSupplyVaultSharesCallback/perf/noExtraPullWhenPercentZero.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyVaultSharesCallback.sol
 * Was:     (no seller pull is emitted when additionalDepositPercent == 0)
 * Now:     SafeTransferLib.safeTransferFrom(loanToken, seller, address(this), sellerAssets);
 * ────────────────────────────────────────────────────────────────────*/
