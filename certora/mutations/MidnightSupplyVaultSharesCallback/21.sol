/* ── MUTATION MidnightSupplyVaultSharesCallback #21 ──────────────────────────────
 * @desc:   Adding one to the additional-deposit amount pulled from the seller overshoots the percent formula by a unit on every positive-percent take, so extraPullMatchesPercentFormula flips to a counterexample.
 * @rules:  extraPullMatchesPercentFormula
 * @conf:   certora/confs/callbacks/MidnightSupplyVaultSharesCallback/perf/extraPullMatchesPercentFormula.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyVaultSharesCallback.sol
 * Was:     amountFromSeller = sellerAssets.mulDivUp(callbackData.additionalDepositPercent, WAD);
 * Now:     amountFromSeller = sellerAssets.mulDivUp(callbackData.additionalDepositPercent, WAD) + 1;
 * ────────────────────────────────────────────────────────────────────*/
