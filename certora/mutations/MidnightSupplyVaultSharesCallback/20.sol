/* ── MUTATION MidnightSupplyVaultSharesCallback #20 ──────────────────────────────
 * @desc:   Supplying the vault shares on behalf of the callback instead of the seller credits the callback's own Midnight position, so the seller's collateral never rises and the witness that a vault-supply take can raise the seller's collateral vanishes.
 ...
 * Now:     MORPHO_MIDNIGHT.supplyCollateral(market, callbackData.collateralIndex, shares, address(this));
 * ────────────────────────────────────────────────────────────────────*/
