/* ── MUTATION MidnightSupplyVaultSharesCallback #5 ──────────────────────────────
 * @desc:   Removing the collateral index validation allows a vault not listed at the configured index to proceed, violating the rule that requires reverts when vault is not at its index
 * @rules:  vaultNotAtIndexReverts
 * @conf:   certora/confs/callbacks/MidnightSupplyVaultSharesCallback/vaultNotAtIndexReverts.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyVaultSharesCallback.sol
 * Was:     CallbackLib.validateVaultCollateral(market, vault, loanToken, callbackData.collateralIndex);
 * Now:     // CallbackLib.validateVaultCollateral(market, vault, loanToken, callbackData.collateralIndex);
 * ────────────────────────────────────────────────────────────────────*/
