/* ── MUTATION BaseMigrationRatifier #28 ──────────────────────────────
 * @desc:   Flips the renewal-window guard from < to >=, so every fixed-source take reverts instead of only early ones; the witness that a post-maturity renewal is executable disappears because the take always reverts.
 * @rules:  postMaturityV2ToV2Executable
 * @conf:   certora/confs/ratifier/reachability.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (block.timestamp < renewalPeriodStart) revert InvalidRenewalWindow();
 * Now:     if (block.timestamp >= renewalPeriodStart) revert InvalidRenewalWindow();
 * ────────────────────────────────────────────────────────────────────*/
