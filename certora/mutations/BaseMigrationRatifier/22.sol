/* ── MUTATION BaseMigrationRatifier #22 ──────────────────────────────
 * @desc:   Tightens the lower duration-band check from < to <=, so a target maturity exactly at now+minDuration is now rejected; the witness that a boundary target maturity is accepted becomes unreachable because the take reverts.
 * @rules:  targetMaturityWithinDurationBand_boundaryAccepted
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (targetMaturity < minTarget || targetMaturity > maxTarget) {
 * Now:     if (targetMaturity <= minTarget || targetMaturity > maxTarget) {
 * ────────────────────────────────────────────────────────────────────*/
