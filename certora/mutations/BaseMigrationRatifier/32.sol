/* ── MUTATION BaseMigrationRatifier #32 ──────────────────────────────
 * @desc:   make-on-behalf check == -> != : real gate uses taker settlementFee, diverging from the fee=0 reconstruction
 * @rules:  borrowerRateGateMatchesNetSellerThreshold, lenderRateGateMatchesNetBuyerThreshold
 * @conf:   certora/confs/ratifier/highlevel.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     uint256 settlementFee = offer.maker == user
 * Now:     uint256 settlementFee = offer.maker != user
 * ────────────────────────────────────────────────────────────────────*/
