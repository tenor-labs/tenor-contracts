/* ── MUTATION BaseMigrationRatifier #19 ──────────────────────────────
 * @desc:   Change the disjunction || to conjunction && so no callback can satisfy the condition, breaking the buy-side flag directionality
 * @rules:  userIsBuyMatchesBuySideCallbacks
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     return callback == LEND_VAULT_TO_MIDNIGHT_CALLBACK || callback == BORROW_MIDNIGHT_TO_BLUE_CALLBACK
 * Now:     return callback == LEND_VAULT_TO_MIDNIGHT_CALLBACK && callback == BORROW_MIDNIGHT_TO_BLUE_CALLBACK
 * ────────────────────────────────────────────────────────────────────*/
