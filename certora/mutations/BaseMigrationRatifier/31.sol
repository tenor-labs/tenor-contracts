/* ── MUTATION BaseMigrationRatifier #31 ──────────────────────────────
 * @desc:   Adds a renewalWindow != 0 revert to the variable-source migration path, so the stored renewal window now gates a V1-to-V2 migration; the assert that such a migration ignores the renewal window flips to a counterexample.
 * @rules:  v1v2migrationsHaveNoRenewalWindowConstraint
 * @conf:   certora/confs/ratifier/highlevel.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (params.renewalCadence == address(0)) revert InvalidRenewalParams();
 * Now:     if (params.renewalCadence == address(0) || params.renewalWindow != 0) revert InvalidRenewalParams();
 * ────────────────────────────────────────────────────────────────────*/
