/* ── MUTATION BaseMigrationRatifier #24 ──────────────────────────────
 * @desc:   Cadence-boundary guard > -> >= : rejects a V1->V2 enter whose nearest boundary is exactly now, a valid config. Caught by the boundary-accepted satisfy companion.
 * @rules:  variableSourceWindowEnforced_boundaryAccepted
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (renewalPeriodStart > block.timestamp) revert InvalidRenewalParams();
 * Now:     if (renewalPeriodStart >= block.timestamp) revert InvalidRenewalParams();
 * ────────────────────────────────────────────────────────────────────*/
