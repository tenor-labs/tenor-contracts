/* ── MUTATION BaseMigrationRatifier #39 ──────────────────────────────
 * @desc:   fee-cap check inverted: over-cap rate can be stored
 * @rules:  feeRateNeverExceedsCallbackCap
 * @conf:   certora/confs/ratifier/valid_state.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (_feeRate > _maxFeeRate(callback)) revert InvalidFeeConfig();
 * Now:     if (_feeRate <= _maxFeeRate(callback)) revert InvalidFeeConfig();
 * ────────────────────────────────────────────────────────────────────*/
