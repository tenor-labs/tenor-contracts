/* ── MUTATION BaseMigrationRatifier #15 ──────────────────────────────
 * @desc:   cadence-grid guard != -> == : accepts off-grid maturities, rejects on-grid
 * @rules:  targetMaturityOnCadenceGrid
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     && IRenewalCadence(params.renewalCadence).cadencePeriodStart(targetMaturity) != targetMaturity
 * Now:     && IRenewalCadence(params.renewalCadence).cadencePeriodStart(targetMaturity) == targetMaturity
 * ────────────────────────────────────────────────────────────────────*/
