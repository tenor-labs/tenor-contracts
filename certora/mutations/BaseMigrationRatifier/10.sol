/* ── MUTATION BaseMigrationRatifier #10 ──────────────────────────────
 * @desc:   Flip || to && to allow single unconfigured field
 * @rules:  unconfiguredTupleAlwaysReverts
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     params.interestRatePolicy == address(0) || params.minDuration == 0
 * Now:     params.interestRatePolicy == address(0) && params.minDuration == 0
 * ────────────────────────────────────────────────────────────────────*/
