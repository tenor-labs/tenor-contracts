/* ── MUTATION BaseMigrationRatifier #40 ──────────────────────────────
 * @desc:   recipient guard == -> != : nonzero rate with zero recipient stored
 * @rules:  nonZeroFeeRateImpliesRecipient
 * @conf:   certora/confs/ratifier/valid_state.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (_feeRate > 0 && _feeRecipient == address(0)) revert InvalidFeeConfig();
 * Now:     if (_feeRate > 0 && _feeRecipient != address(0)) revert InvalidFeeConfig();
 * ────────────────────────────────────────────────────────────────────*/
