/* ── MUTATION BaseMigrationRatifier #21 ──────────────────────────────
 * @desc:   Replace zeroFloorSub with plain subtraction to allow underflow on exits after source maturity, violating the clamping to 0 invariant
 * @rules:  computeDurationPerCallback
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     return UtilsLib.zeroFloorSub(sourceMaturity, block.timestamp);
 * Now:     return sourceMaturity - block.timestamp;
 * ────────────────────────────────────────────────────────────────────*/
