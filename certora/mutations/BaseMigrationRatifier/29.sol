/* ── MUTATION BaseMigrationRatifier #29 ──────────────────────────────
 * @desc:   Inverts the target-maturity guard from >0 to ==0, so an exit with zero target maturity now runs target-maturity validation and always reverts; the witness that a post-maturity exit is executable disappears because the take reverts.
 * @rules:  postMaturityV2ToV1Executable
 * @conf:   certora/confs/ratifier/reachability.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (targetMaturity > 0) _validateTargetMaturity(sourceMaturity, targetMaturity, params);
 * Now:     if (targetMaturity == 0) _validateTargetMaturity(sourceMaturity, targetMaturity, params);
 * ────────────────────────────────────────────────────────────────────*/
