/* ── MUTATION BaseMigrationRatifier #25 ──────────────────────────────
 * @desc:   tick guard != -> == : reverts on a matching tick, accepts a mismatch
 * @rules:  tickMustMatchOffer
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (decoded.tick != offer.tick) revert InvalidCallbackData();
 * Now:     if (decoded.tick == offer.tick) revert InvalidCallbackData();
 * ────────────────────────────────────────────────────────────────────*/
