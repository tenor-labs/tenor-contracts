/* ── MUTATION BaseMigrationRatifier #23 ──────────────────────────────
 * @desc:   Window param guard > -> >= : rejects renewalWindow == sourceMaturity (window opens at time 0), a valid config. Caught by the boundary-accepted satisfy companion.
 * @rules:  v2SourceWindowEnforcedBeforeOpen_boundaryAccepted
 * @conf:   certora/confs/ratifier/revert.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (params.renewalWindow > sourceMaturity) revert InvalidRenewalParams();
 * Now:     if (params.renewalWindow >= sourceMaturity) revert InvalidRenewalParams();
 * ────────────────────────────────────────────────────────────────────*/
