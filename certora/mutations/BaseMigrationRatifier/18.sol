/* ── MUTATION BaseMigrationRatifier #18 ──────────────────────────────
 * @desc:   Flip the condition from != to == to invert the override logic; market config is returned even when not set, instead of falling back to default
 * @rules:  getEffectiveFeeConfigMarketOverridesActionDefault
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (config.feeRecipient != address(0)) return config;
 * Now:     if (config.feeRecipient == address(0)) return config;
 * ────────────────────────────────────────────────────────────────────*/
