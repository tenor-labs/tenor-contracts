/* ── MUTATION BaseMigrationRatifier #46 ──────────────────────────────
 * @desc:   target-maturity guard <= -> >= : V1->V2 (srcMat==0) always reverts, witness unreachable
 * @rules:  entryV1ToV2Executable
 * @conf:   certora/confs/ratifier/reachability.conf
 * @status: killed
 * @target: src/ratifiers/BaseMigrationRatifier.sol
 * Was:     if (targetMaturity <= sourceMaturity) revert InvalidTargetMaturity();
 * Now:     if (targetMaturity >= sourceMaturity) revert InvalidTargetMaturity();
 * ────────────────────────────────────────────────────────────────────*/
