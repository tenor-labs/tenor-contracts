/* ── MUTATION BorrowMidnightRenewalCallback #25 ──────────────────────────────
 * @desc:   onSell inserts safeTransferFrom(units+1) into the callback : loanToken inflow exceeds units bound
 * @rules:  renewalCallbackNeverPullsExternalLoanToken
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/perf/renewalCallbackNeverPullsExternalLoanToken.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightRenewalCallback.sol
 * Was:     (the original callback body has no such operation here)
 * Now:     SafeTransferLib.safeTransferFrom(market.loanToken, seller, address(this), units + 1);
 * ────────────────────────────────────────────────────────────────────*/
