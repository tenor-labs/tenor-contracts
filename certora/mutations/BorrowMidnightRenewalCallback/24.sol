/* ── MUTATION BorrowMidnightRenewalCallback #24 ──────────────────────────────
 * @desc:   The callback inserts an extra self-funded repayment on the target market, so debt drops on both the source and target markets, and the rule requiring at most one market's debt to fall reports a counterexample.
 * @rules:  renewalReducesDebtOnAtMostOneMarket
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/perf/renewalReducesDebtOnAtMostOneMarket.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightRenewalCallback.sol
 * Was:     (the original callback body has no such operation here)
 * Now:     IERC20(market.loanToken).forceApprove(address(MORPHO_MIDNIGHT), units + 1);
