/* ── MUTATION BorrowMidnightRenewalCallback #36 ──────────────────────────────
 * @desc:   Doubling the seller fee transfer pushes the fee recipient's loanToken balance delta past the interest-share bound, so borrowerFeeBoundedByInterestShare flips to a counterexample.
 * @rules:  borrowerFeeBoundedByInterestShare
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/perf_kill/borrowerFeeBoundedByInterestShare.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightRenewalCallback.sol
 * Was:     SafeTransferLib.safeTransfer(market.loanToken, callbackData.feeRecipient, fee);
 * Now:     SafeTransferLib.safeTransfer(market.loanToken, callbackData.feeRecipient, fee * 2);
 * ────────────────────────────────────────────────────────────────────*/
