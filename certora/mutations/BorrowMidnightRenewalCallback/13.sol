/* ── MUTATION BorrowMidnightRenewalCallback #13 ──────────────────────────────
 * @desc:   onSell receiver guard flipped (!= -> ==): receiver!=callback no longer reverts. Caught by receiverNotCallbackReverts (CLB-BMR-13, receiverNotCallback => reverted via callbackCallWithRevert) — the flip leaves a non-reverting receiver!=callback trace, assert violated.
 * @rules:  receiverNotCallbackReverts
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/receiverNotCallbackReverts.conf
 * @status: killed
