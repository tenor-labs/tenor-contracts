/* ── MUTATION BorrowMidnightRenewalCallback #31 ──────────────────────────────
 * @desc:   The loan-token match check is inverted so that matching source and target tokens revert, which every real renewal needs, so take() always reverts and the witness opening new target debt without removing collateral can no longer be produced.
 * @rules:  renewalCannotRemoveCollateralWhenOpeningDebt__satisfy
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/debug_satisfy/renewalCannotRemoveCollateralWhenOpeningDebt.conf
 * @status: killed
 * @target: src/callbacks/BorrowMidnightRenewalCallback.sol
 * Was:     if (callbackData.sourceMarket.loanToken != market.loanToken) revert CallbackLib.TokenMismatch();
 * Now:     if (callbackData.sourceMarket.loanToken == market.loanToken) revert CallbackLib.TokenMismatch();
 * ────────────────────────────────────────────────────────────────────*/
