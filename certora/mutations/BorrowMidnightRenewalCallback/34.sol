/* ── MUTATION BorrowMidnightRenewalCallback #34 ──────────────────────────────
 * @desc:   Flipping the Midnight-caller guard to == makes onSell revert on its first instruction whenever the caller is Midnight, which it always is inside take(), so the renewalAddsDebtOnAtMostOneMarket satisfy witness can never reach its assert point and turns unsatisfiable.
 * @rules:  renewalAddsDebtOnAtMostOneMarket__satisfy
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/perf_satisfy/renewalAddsDebtOnAtMostOneMarket.conf
 * @status: killed
