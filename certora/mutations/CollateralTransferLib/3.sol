/* ── MUTATION CollateralTransferLib #3 ───────────────────────
 * @desc:   the inlined collateral loop withdraws collateralToTransfer-1 from source but supplies the full amount to target : moves MORE collateral than withdrawn (callback seed funds +1)
 * @rules:  renewalCannotMoveMoreCollateralThanWithdrawn
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/perf_kill/renewalCannotMoveMoreCollateralThanWithdrawn.conf
 * @status: killed
 * @target: src/libraries/CollateralTransferLib.sol
 * Was:     morphoMidnight.withdrawCollateral(sourceMarket, i, collateralToTransfer, borrower, address(this));
 * Now:     morphoMidnight.withdrawCollateral(sourceMarket, i, collateralToTransfer - 1, borrower, address(this));
 * ────────────────────────────────────────────────────────────*/
