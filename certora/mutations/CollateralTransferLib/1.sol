/* ── MUTATION CollateralTransferLib #1 ──────────────────────────────
 * @desc:   Subtracts 1 from the source collateral amount read on the closing fill, so the source position can never be fully drained; the renewalCanFullyCloseOldPosition witness requiring collateral to reach zero becomes unsatisfiable.
 * @rules:  renewalCanFullyCloseOldPosition
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/perf_kill/renewalCanFullyCloseOldPosition.conf
 * @status: killed
 * @target: src/libraries/CollateralTransferLib.sol
 * Was:     uint256 collateralToTransfer = morphoMidnight.collateral(sourceMarketId, borrower, i);
 * Now:     uint256 collateralToTransfer = morphoMidnight.collateral(sourceMarketId, borrower, i) - 1;
 * ────────────────────────────────────────────────────────────────────*/
