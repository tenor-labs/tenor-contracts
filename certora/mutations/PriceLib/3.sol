/* ── MUTATION PriceLib #3 ──────────────────────────────
 * @desc:   computeEffectiveRate buy-side max->min (> to <)
 * @rules:  effectiveRateSelectsTighterBound
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/libraries/PriceLib.sol
 * Was:     ? (policyRate > limitRate ? policyRate : limitRate)
 * Now:     ? (policyRate < limitRate ? policyRate : limitRate)
 * ────────────────────────────────────────────────────────────────────*/
