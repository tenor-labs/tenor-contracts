/* ── MUTATION PriceLib #7 ──────────────────────────────
 * @desc:   borrower compare >= -> <= (copy-paste of lender branch): inverts limit-monotonicity [same diff as PriceLib#8, re-proven under highlevel.conf]
 * @rules:  satisfiesRateLimitMonotoneInBorrowerLimit, satisfiesRateLimitComparisonDirection
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/libraries/PriceLib.sol
 * Was:     return assets * WAD >= units * price;
 * Now:     return assets * WAD <= units * price;
 * ────────────────────────────────────────────────────────────────────*/
