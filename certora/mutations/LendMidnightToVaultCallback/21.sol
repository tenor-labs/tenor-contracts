/* ── MUTATION LendMidnightToVaultCallback #21 ──────────────────────────────
 * @desc:   onSell inserts foreign withdrawCollateral on seller : mutates collateral[seller][0], breaks collateral-unchanged
 * @rules:  vaultExitLeavesCollateralUnchanged
 * @conf:   certora/confs/callbacks/LendMidnightToVaultCallback/vaultExitLeavesCollateralUnchanged.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightToVaultCallback.sol
 * Was:     (the original callback body has no such operation here)
 * Now:     MORPHO_MIDNIGHT.withdrawCollateral(market, 0, 1, seller, address(this));
 * ────────────────────────────────────────────────────────────────────*/
