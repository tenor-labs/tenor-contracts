/* ── MUTATION LendMidnightToVaultCallback #7 ──────────────────────────────
 * @desc:   The vault deposit is redirected from the seller to the zero address, which reverts as a mint-to-zero on every fill, so take() always reverts and the satisfiability witness showing a lender's credit can be fully closed becomes unsatisfiable.
 * @rules:  vaultExitCanFullyCloseCredit
 * @conf:   certora/confs/callbacks/LendMidnightToVaultCallback/vaultExitCanFullyCloseCredit.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightToVaultCallback.sol
 * Was:     uint256 shares = IERC4626(callbackData.vault).deposit(depositAmount, seller);
 * Now:     uint256 shares = IERC4626(callbackData.vault).deposit(depositAmount, address(0));
 * ────────────────────────────────────────────────────────────────────*/
