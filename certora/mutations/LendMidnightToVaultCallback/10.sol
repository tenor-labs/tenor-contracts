/* ── MUTATION LendMidnightToVaultCallback #10 ──────────────────────────────
 * @desc:   Doubling the percentage fee makes 100 * fee > assets, exceeding the 1% cap and violating the assertion 100 * fee <= assets.
 * @rules:  percentageFeeNeverExceedsAssets
 * @conf:   certora/confs/callbacks/LendMidnightToVaultCallback/percentageFeeNeverExceedsAssets.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightToVaultCallback.sol
 * Was:     fee = CallbackLib.percentageFee(sellerAssets, callbackData.feeRate);
 * Now:     fee = CallbackLib.percentageFee(sellerAssets, callbackData.feeRate) * 2;
 * ────────────────────────────────────────────────────────────────────*/
