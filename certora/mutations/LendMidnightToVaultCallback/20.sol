/* ── MUTATION LendMidnightToVaultCallback #20 ──────────────────────────────
 * @desc:   onSell inserts safeTransfer(collateralParams[0].token, Midnight, 1) : moves a non-loanToken, non-vault token with no settlement-fee delta
 * @rules:  vaultExitConservesMidnightBalanceMinusFee
 * @conf:   certora/confs/callbacks/LendMidnightToVaultCallback/vaultExitConservesMidnightBalanceMinusFee.conf
 * @status: killed
