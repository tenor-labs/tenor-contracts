/* ── MUTATION LendMidnightRenewalCallback #23 ──────────────────────────────
 * @desc:   Inserts an extra transfer that pulls units+1 of loan token directly from the buyer on top of the legitimate withdraw, so the callback's net external inflow exceeds units and the rule bounding that inflow flips to a counterexample.
 * @rules:  renewalCallbackNeverPullsExternalLoanToken
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/perf/renewalCallbackNeverPullsExternalLoanToken.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightRenewalCallback.sol
 * Was:     (onBuy performs no external loanToken pull here)
 * Now:     IERC20(market.loanToken).transferFrom(buyer, address(this), units + 1);  // inserted after forceApprove
 * ────────────────────────────────────────────────────────────────────*/
