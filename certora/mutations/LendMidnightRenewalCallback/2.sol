/* ── MUTATION LendMidnightRenewalCallback #2 ──────────────────────────────
 * @desc:   zero-amount guard || -> &&
 * @rules:  callbackRevertsOnZeroAssetsOrUnits
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/perf/callbackRevertsOnZeroAssetsOrUnits.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightRenewalCallback.sol
 * Was:     if (buyerAssets == 0 || units == 0) revert CallbackLib.ZeroAmount();
 * Now:     if (buyerAssets == 0 && units == 0) revert CallbackLib.ZeroAmount();
 * ────────────────────────────────────────────────────────────────────*/
