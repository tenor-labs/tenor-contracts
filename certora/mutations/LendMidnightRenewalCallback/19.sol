/* ── MUTATION LendMidnightRenewalCallback #19 ──────────────────────────────
 * @desc:   onBuy inserts a 2nd MORPHO_MIDNIGHT.withdraw(units+buyerAssets+1) overshooting take's +units target-credit deposit : credit net-drops on both source and target
 * @rules:  renewalReducesCreditOnAtMostOneMarket
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/perf_kill/renewalReducesCreditOnAtMostOneMarket.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightRenewalCallback.sol
 * Was:     (the original callback body has no such operation here)
 * Now:     MORPHO_MIDNIGHT.withdraw(market, units + buyerAssets + 1, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
