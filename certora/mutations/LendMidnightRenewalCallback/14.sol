/* ── MUTATION LendMidnightRenewalCallback #14 ──────────────────────────────
 * @desc:   lost the WAD denominator (effPrice,1): fee = units*effPrice explodes far past units
 * @rules:  buyerTickFeePaidBoundedByUnits
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/buyerTickFeePaidBoundedByUnits.conf
 * @status: killed
 * @target: src/libraries/CallbackLib.sol
 * Was:     return units.mulDivDown(effPrice, WAD).zeroFloorSub(assets);
 * Now:     return units.mulDivDown(effPrice, 1).zeroFloorSub(assets);
 * ────────────────────────────────────────────────────────────────────*/
