/* ── MUTATION LendMidnightRenewalCallback #25 ──────────────────────────────
 * @desc:   Approves one less than buyerAssets for settlement, so Midnight's pull of the full buyerAssets exceeds the allowance and reverts on every path, making take() always revert and leaving the witness unsatisfiable.
 * @rules:  renewalNeverTouchesUnrelatedLenderCredit__satisfy
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/perf_kill/renewalNeverTouchesUnrelatedLenderCredit.conf
 * @status: killed
