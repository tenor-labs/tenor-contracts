/* ── MUTATION LendMidnightRenewalCallback #16 ──────────────────────────────
 * @desc:   source withdraw zeroed: the lender's source-market credit can never reach zero, so the position-bound close witness goes UNSAT (VIOLATED = killed).
 * @rules:  renewalCanFullyCloseOldCredit
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/perf_kill/renewalCanFullyCloseOldCredit.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightRenewalCallback.sol
 * Was:     MORPHO_MIDNIGHT.withdraw(callbackData.sourceMarket, withdrawAssets, buyer, address(this));
 * Now:     MORPHO_MIDNIGHT.withdraw(callbackData.sourceMarket, 0, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
