/* ── MUTATION LendMidnightRenewalCallback #21 ──────────────────────────────
 * @desc:   Inverts the zero-credit guard from ==0 to !=0, so every renewal with source credit reverts and the only admitted path (zero source credit) then fails the insufficient-credit check, making take() revert on all paths and leaving the reachability witness unsatisfiable.
 * @rules:  renewalAddsCreditOnAtMostOneMarket__satisfy
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/perf_satisfy/renewalAddsCreditOnAtMostOneMarket.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightRenewalCallback.sol
 * Was:     if (sourceCredit == 0) revert CallbackLib.ZeroAmount();
 * Now:     if (sourceCredit != 0) revert CallbackLib.ZeroAmount();
 * ────────────────────────────────────────────────────────────────────*/
