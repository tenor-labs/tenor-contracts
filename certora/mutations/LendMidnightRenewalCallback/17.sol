/* ── MUTATION LendMidnightRenewalCallback #17 ──────────────────────────
 * @desc:   Forces the buyer callback fee to zero, so the fee recipient is never paid even though the credit still rolls, and the witness that requires both a credit roll and a positive fee payment can no longer be satisfied.
 * @rules:  renewalCanMoveCreditWithPositiveFee
 * @conf:   certora/confs/callbacks/LendMidnightRenewalCallback/renewalCanMoveCreditWithPositiveFee.conf
 * @status: killed
 * @target: src/callbacks/LendMidnightRenewalCallback.sol
 * Was:     uint256 fee = CallbackLib.buyerFeeFromTick(callbackData.tick, callbackData.feeRate, units, buyerAssets);
 * Now:     uint256 fee = 0;
 * ────────────────────────────────────────────────────────────────────*/
