/* ── MUTATION LendVaultToMidnightCallback #12 ──────────────────────────────
 * @desc:   Inserts an extra Midnight withdraw call that reduces the fee recipient's credit by one on a successful take, changing an unrelated user's balance and flipping the unrelated-user-untouched assert to a counterexample.
 * @rules:  vaultFundedLendNeverTouchesUnrelatedUser
 * @conf:   certora/confs/callbacks/LendVaultToMidnightCallback/vaultFundedLendNeverTouchesUnrelatedUser.conf
 * @status: killed
 * @target: src/callbacks/LendVaultToMidnightCallback.sol
 * Was:     (the original callback body has no such operation here)
 * Now:     MORPHO_MIDNIGHT.withdraw(market, 1, callbackData.feeRecipient, address(this));
 * ────────────────────────────────────────────────────────────────────*/
