/* ── MUTATION LendVaultToMidnightCallback #7 ──────────────────────────────
 * @desc:   onBuy inserts foreign withdrawCollateral on buyer : mutates collateral[buyer][0], breaks collateral-unchanged
 * @rules:  vaultFundedLendLeavesCollateralUnchanged
 * @conf:   certora/confs/callbacks/LendVaultToMidnightCallback/vaultFundedLendLeavesCollateralUnchanged.conf
 * @status: killed
 * @target: src/callbacks/LendVaultToMidnightCallback.sol
 * Was:     (the original callback body has no such operation here)
 * Now:     MORPHO_MIDNIGHT.withdrawCollateral(market, 0, 1, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
