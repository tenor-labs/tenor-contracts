/* ── MUTATION LendVaultToMidnightCallback #4 ──────────────────────────────
 * @desc:   Inverts the token check from != to ==, so a vault whose asset does not match the market loan token is accepted instead of rejected; the mismatched-vault call no longer reverts and the assert flips to a counterexample.
 * @rules:  vaultAssetMismatchReverts
 * @conf:   certora/confs/callbacks/LendVaultToMidnightCallback/vaultAssetMismatchReverts.conf
 * @status: killed
