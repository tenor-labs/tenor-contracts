/* ── MUTATION MidnightWithdrawVaultSharesCallback #8 ──────────────────────────────
 * @desc:   The callback approves Midnight for zero loanToken instead of buyerAssets, so Midnight cannot pull the funds and take() reverts, leaving the rule unable to witness a successful withdraw fill.
 * @rules:  takeLeavesVaultShareBalanceUnchanged__satisfy
 * @conf:   certora/confs/callbacks/MidnightWithdrawVaultSharesCallback/debug_satisfy/takeLeavesVaultShareBalanceUnchanged.conf
 * @status: killed
 ...
 * Now:     IERC20(market.loanToken).forceApprove(msg.sender, 0);
 * ────────────────────────────────────────────────────────────────────*/
