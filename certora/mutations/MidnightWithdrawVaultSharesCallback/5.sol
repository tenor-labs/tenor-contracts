/* ── MUTATION MidnightWithdrawVaultSharesCallback #5 ──────────────────────────────
 * @desc:   Withdrawing 0 collateral instead of the computed amount prevents collateral reduction; the assertion that collateral < collateralBefore fails.
 * @rules:  takeCanDropCollateralOnNarrowedMarket
 * @conf:   certora/confs/callbacks/MidnightWithdrawVaultSharesCallback/takeCanDropCollateralOnNarrowedMarket.conf
 * @status: killed
 * @target: src/callbacks/MidnightWithdrawVaultSharesCallback.sol
 * Was:     MORPHO_MIDNIGHT.withdrawCollateral(market, callbackData.collateralIndex, sharesToWithdraw, buyer, address(this));
 * Now:     MORPHO_MIDNIGHT.withdrawCollateral(market, callbackData.collateralIndex, 0, buyer, address(this));
 * ────────────────────────────────────────────────────────────────────*/
