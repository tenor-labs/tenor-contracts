/* ── MUTATION MidnightWithdrawVaultSharesCallback #6 ──────────────────────────────
 * @desc:   Withdrawing only half the assets leaves the callback holding half of the vault shares; the assertion that callback balance == 0 fails.
 * @rules:  callbackNeverHoldsTokens
 * @conf:   certora/confs/callbacks/MidnightWithdrawVaultSharesCallback/callbackNeverHoldsTokens.conf
 * @status: killed
 * @target: src/callbacks/MidnightWithdrawVaultSharesCallback.sol
 * Was:     IERC4626(callbackData.vault).withdraw(buyerAssets, address(this), address(this));
 * Now:     IERC4626(callbackData.vault).withdraw(buyerAssets / 2, address(this), address(this));
 * ────────────────────────────────────────────────────────────────────*/
