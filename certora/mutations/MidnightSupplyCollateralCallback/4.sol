/* ── MUTATION MidnightSupplyCollateralCallback #4 ──────────────────────────────
 * @desc:   Removes the length mismatch check, allowing amounts[] array with wrong length to bypass validation
 * @rules:  collateralLengthMismatchReverts
 * @conf:   certora/confs/callbacks/MidnightSupplyCollateralCallback/collateralLengthMismatchReverts.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyCollateralCallback.sol
 * Was:     if (callbackData.amounts.length != collateralsLength) revert CallbackLib.InvalidCollateral();
 * Now:     // if (callbackData.amounts.length != collateralsLength) revert CallbackLib.InvalidCollateral();
 * ────────────────────────────────────────────────────────────────────*/
