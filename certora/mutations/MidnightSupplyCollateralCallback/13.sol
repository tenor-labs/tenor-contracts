/* ── MUTATION MidnightSupplyCollateralCallback #13 ──────────────────────────────
 * @desc:   supply amount zeroed: no collateral ever reaches the seller, so the max-capacity fill witness goes UNSAT (VIOLATED = killed).
 * @rules:  maxBorrowCapacityUsageFillReachable
 * @conf:   certora/confs/callbacks/MidnightSupplyCollateralCallback/maxBorrowCapacityUsageFillReachable.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyCollateralCallback.sol
 * Was:     MORPHO_MIDNIGHT.supplyCollateral(market, i, supplyAmount, seller);
 * Now:     MORPHO_MIDNIGHT.supplyCollateral(market, i, 0, seller);
 * ────────────────────────────────────────────────────────────────────*/
