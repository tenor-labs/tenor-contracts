/* ── MUTATION MidnightSupplyCollateralCallback #14 ──────────────────────────────
 * @desc:   Changes the zero-amount guard to reject 1 instead of 0, so a zero offerSellerAssets denominator is now accepted; the rule requiring a zero offerSellerAssets to revert is violated.
 * @rules:  offerSellerAssetsZeroReverts
 * @conf:   certora/confs/callbacks/MidnightSupplyCollateralCallback/offerSellerAssetsZeroReverts.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyCollateralCallback.sol
 * Was:     if (callbackData.offerSellerAssets == 0) revert CallbackLib.ZeroAmount();
 * Now:     if (callbackData.offerSellerAssets == 1) revert CallbackLib.ZeroAmount();
 * ────────────────────────────────────────────────────────────────────*/
