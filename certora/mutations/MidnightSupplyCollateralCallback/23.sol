/* ── MUTATION MidnightSupplyCollateralCallback #23 ──────────────────────────────
 * @desc:   Flips the cap check from greater-than to less-than, so a borrow-capacity usage above the maximum no longer reverts; the rule asserting usage stays within the cap is violated on the non-reverting path.
 * @rules:  borrowCapacityUsageWithinCap
 * @conf:   certora/confs/callbacks/MidnightSupplyCollateralCallback/borrowCapacityUsageWithinCap.conf
 * @status: killed
 * @target: src/callbacks/MidnightSupplyCollateralCallback.sol
 * Was:     if (borrowCapacityUsage > callbackData.maxBorrowCapacityUsage) {
 * Now:     if (borrowCapacityUsage < callbackData.maxBorrowCapacityUsage) {
 * ────────────────────────────────────────────────────────────────────*/
