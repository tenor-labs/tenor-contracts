/* ── MUTATION CallbackLib #3 ──────────────────────────────
 * @desc:   transposed mulDivDown args (feeRate,WAD)->(WAD,feeRate): fee share inverse in feeRate breaks net-price fee-monotonicity
 * @rules:  netSellerPriceMonotoneInFee, netBuyerPriceMonotoneInFee
 * @conf:   certora/confs/ratifier/unit.conf
 * @status: killed
 * @target: src/libraries/CallbackLib.sol
 * Was:     return (WAD - price).mulDivDown(feeRate, WAD);
 * Now:     return (WAD - price).mulDivDown(WAD, feeRate);
 * ────────────────────────────────────────────────────────────────────*/
