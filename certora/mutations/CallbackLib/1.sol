/* ── MUTATION CallbackLib #1 ──────────────────────────────
 * @desc:   _interestFeeComponent sign flip (WAD-price)->(WAD+price): nonzero interest fee at par, so the tick fee no longer vanishes
 * @rules:  tickFeeVanishesAtPar
 * @conf:   certora/confs/callbacks/LendVaultToMidnightCallback/tickFeeVanishesAtPar.conf
 * @status: killed
