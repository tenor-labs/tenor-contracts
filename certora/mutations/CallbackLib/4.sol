/* ── MUTATION CallbackLib #4 ──────────────────────────────
 * @desc:   _interestFeeComponent sign flip (WAD-price)->(WAD+price): nonzero interest fee at par, so the tick fee no longer vanishes; re-proves the kill for the BMR instance (the CallbackLib #1 kill under the LVM conf is not evidence for this per-(contract,rule) instance).
 * @rules:  tickFeeVanishesAtPar
 * @conf:   certora/confs/callbacks/BorrowMidnightRenewalCallback/tickFeeVanishesAtPar.conf
 * @status: killed
