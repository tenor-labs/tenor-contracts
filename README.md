...
- **Continuous-fee protection** - `maxContinuousFee` reverts the batch with `ContinuousFeeAboveMax` if the market's continuous fee exceeds it, checked once after touching the batch market (so a market created by the batch is bounded on its `defaultContinuousFee`). Protects the taker against fee increases between signing and execution; `type(uint256).max` disables
